<div id="content_api_merlion">
    {* $api_merlion_order_products_status|fn_print_r *}
    <table width="100%" class="table table-middle">
    <thead>
        <tr>
            <th width="50%">Работа с заказом</th>
            <th width="10%"></th>
            <th class="center" width="10%"></th>
            <th class="center" width="10%"></th>
            <th class="center" width="10%">{include file="buttons/button.tpl" but_role="action" but_meta="apim-button-status-test" but_onclick="Tygh.$.ceAjax('request', '{"api_merlion_orders.order_check_products?order_id=`$order_info.order_id`"|fn_url nofilter}', {$ldelim}result_ids: 'api_merlion_order_products'{$rdelim})" allow_href=false but_target_id="api_merlion_order_products"}<i class="cm-tooltip icon-question-sign" title="{__('api_merlion_tooltip.order_get_status_products')}"></i></th>
            <th width="10%" class="right">{include file="buttons/button.tpl" but_role="action" but_meta="apim-button-all-to-order" but_onclick="Tygh.$.ceAjax('request', '{"api_merlion_orders.order_add_all_products?order_id=`$order_info.order_id`"|fn_url nofilter}', {$ldelim}result_ids: 'api_merlion_order_products'{$rdelim})" allow_href=false but_target_id="api_merlion_order_products"}<i class="cm-tooltip icon-question-sign" title="{__('api_merlion_tooltip.order_reserve_all_products')}"></i></th>
        </tr>
    </thead>
